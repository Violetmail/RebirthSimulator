...
![image](https://github.com/Violetmail/RebirthSimulator/blob/master/APPandDisplay/Activity.png))
...
![image](https://github.com/Violetmail/RebirthSimulator/blob/master/APPandDisplay/Database.png)
### 表构成
...
### 抽卡
抽卡功能主要集中在概率的分配，定义抽卡函数，通过调用随机数函数实现一次抽卡或者十连抽。同时抽卡后将改变各类属性，比如货币数量，保底次数等，均和数据库产生连接，并每次抽卡后执行界面刷新。在水晶的显示框旁边有使用水晶券的按钮，每次使用会弹出对话框确认，使用后会立刻改变相关数据以及界面刷新。
...
- APPandDisplay文件夹中存放项目的apk安装包和演示视频。
